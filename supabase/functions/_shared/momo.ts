 ...
 *
 * Token acquisition below is the documented sandbox/production flow and works
 * as-is. The statement endpoint is deliberately configurable: which endpoint
 * returns "payments received on date X" depends on the MoMo product the
 * business is approved for, and that approval (registered business + KYC) can
 * take weeks. Until it lands, point MOMO_BASE_URL at the sandbox and the rest
 * of the pipeline — matching, variance, reporting — runs end to end on
 * sandbox data.
 *
 * Env:
...
async function getAccessToken(config: MomoConfig): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error(`MoMo token request failed: ${response.status} ${await response.text()}`);
  }
...
export async function fetchMomoPayments(
  ...
  if (!response.ok) {
    throw new Error(`MoMo statement request failed: ${response.status} ${await response.text()}`);
  }

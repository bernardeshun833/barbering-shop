...
/**
 * Pulls recent MoMo payments and upserts them on external_ref, so the same
 * payment arriving in two overlapping windows lands once. Runs every 15
 * minutes (see migration 0005) with a deliberately generous lookback — the
 * cost of re-fetching a payment we already have is nothing, and the cost of
 * missing one is a false "money arrived with no sale logged" flag at 21:30.
 */

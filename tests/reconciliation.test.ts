...
describe("Check A — digital declared vs digital received", () => {
  it("matches a MoMo payment to the digital sale at the same amount and time", () => {
  ...
  it("flags money received with no sale logged as HIGH", () => {
  ...
  it("flags a digital sale whose money never arrived as HIGH", () => {
    const report = reconcile(
  ...
  it("pairs each MoMo payment with a distinct sale when amounts are identical", () => {
  ...
  it("escalates to HIGH when the digital variance exceeds 1%", () => {
    // Declared 100 digital, 90 actually received — matched rows do not make
    // the totals agree, and the percentage check is what catches it.
  ...
  it("treats a QR sale as matchable against MoMo data", () => {
    const sale = txn({ amount: 25, payment_method: "qr", created_at_local: at("12:00") });
    const report = reconcile(
      input({ posTransactions: [sale], momoPayments: [momo({ amount: 25, timestamp: at("12:03") })] })
    );
...
describe("Check B — cash declared vs cash counted", () => {
  it("computes expected cash as cash sales plus the opening float", () => {
    const report = reconcile(
      input({
  ...
  it("ignores the expected figure submitted by the device", () => {
    // The tablet could send any `expected` it likes; the server recomputes.
    const report = reconcile(
      input({
...
describe("Check C — volume sanity", () => {
  ...
  it("flags a barber below 70% of their own usual volume", () => {
    const report = reconcile(
      input({
        posTransactions: [txn({ barber_id: "barber-1" })],
        cashCount: cashCount(240),
        baseline: {
          sameWeekdayCounts: [],
          barberDailyCounts: { "barber-1": [8, 9, 10, 8, 9] },
          cashRatios: []
        }
      })
    );

    expect(kinds(report.flags)).toContain("barber_volume_drop");
    expect(report.severity).toBe("LOW");
  });

  it("does not flag a barber holding near their usual volume", () => {
    const report = reconcile(
      input({
    ...
    expect(kinds(report.flags)).not.toContain("barber_volume_drop");
  });
});

describe("Check D — behavioural anomalies", () => {
  ...
  it("flags a transaction logged after closing time", () => {
    const report = reconcile(
      input({
        posTransactions: [txn({ created_at_local: at("22:30") })],
        cashCount: cashCount(240)
      })
    );
  ...
  it("does not flag transactions inside opening hours", () => {
    const report = reconcile(
      input({
        posTransactions: [txn({ created_at_local: at("08:00") }), txn({ created_at_local: at("19:59") })],
...
describe("severity", () => {
    ...
    const report = reconcile(
      input({
        ...
        baseline: {
          ...
          barberDailyCounts: { "barber-1": [2, 2, 3] },
  ...
  it("takes the highest severity present, not the most recent", () => {
    const report = reconcile(
      input({
        posTransactions: [txn({ amount: 100, created_at_local: at("23:00") })],
...
describe("corrections", () => {
  it("nets a void out of revenue and out of the transaction count", () => {
  ...
  it("surfaces MoMo money that arrived for a sale that was later voided", () => {
    const original = txn({
      amount: 60,
      payment_method: "momo",
...
describe("per-barber breakdown", () => {
  it("splits revenue by barber and payment type", () => {
    const report = reconcile(
      input({
        posTransactions: [
          txn({ barber_id: "barber-1", amount: 40, payment_method: "cash" }),
          txn({ barber_id: "barber-1", amount: 60, payment_method: "momo", created_at_local: at("11:00") }),
          txn({ barber_id: "barber-2", amount: 25, payment_method: "cash" })
        ],
    ...
    const first = report.per_barber.find((b) => b.barber_id === "barber-1")!;
    ...
    expect(report.per_barber[0].barber_id).toBe("barber-1"); // sorted by revenue
  });
});
...
describe("computeDeviceGaps", () => {
  it("measures silence from opening time to the first sync", () => {
    const gaps = computeDeviceGaps(
      [txn({ device_id: "tablet-1", synced_at: at("14:00") })],
      [],
      DATE,
      "08:00",
      "20:00"
  ...
  it("measures silence from the last sync to closing time", () => {
    const gaps = computeDeviceGaps(
      [
        txn({ device_id: "tablet-1", synced_at: at("08:30") }),
        txn({ device_id: "tablet-1", synced_at: at("09:00") })
      ],
      [],
      DATE,
      "08:00",
  ...
  it("counts heartbeats as proof of life even with no sales to push", () => {
    const gaps = computeDeviceGaps(
      [],
      [
        { device_id: "tablet-1", last_synced_at: at("12:00") },
        { device_id: "tablet-1", last_synced_at: at("16:00") }
      ],
      DATE,
      "08:00",
      "20:00"
  ...
  it("reports a full day of silence for a device that never synced", () => {
    const gaps = computeDeviceGaps(
      [],
      [{ device_id: "tablet-1", last_synced_at: `${DATE}T23:00:00.000Z` }],
      DATE,
      "08:00",
      "20:00"
    );
